...
#   omp-ext          omp per-task extension (agent_start/agent_end/turn_end; omp
#                    has no agent_settled event, so idle is agent_end when the
#                    event is terminal, not ctx.isIdle)

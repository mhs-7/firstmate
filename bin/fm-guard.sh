...
# polling is active (state/x-watch.check.sh exists) and the watcher's liveness
# beacon (state/.last-watcher-beat, touched every poll cycle) is missing or
# stale beyond FM_GUARD_GRACE seconds, prints a loud, clearly delimited
...
# grace-based predicate (bin/fm-supervision-lib.sh). The guard is called during
# ordinary handling turns, when the one-shot watcher can be between cycles, so
# the beacon grace predicate is authoritative here; the turn-end guard retains
# the stricter live-lock check for blocking a turn end.
...
# No fresh watcher beacon with work needing supervision is the dangerous state:
# emit a prominent,

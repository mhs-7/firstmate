...
| codex | `--model <model>` | `-c 'model_reasoning_effort="<low\|medium\|high\|xhigh\|max>"'` | Verified 2026-08-10 on codex-cli 0.147.0. `codex debug models` reports `low`, `medium`, `high`, `xhigh`, `max`, and `ultra` for the active model. The exact command `printf 'Do not use tools. Reply OK.\\n' | codex exec --ephemeral --ignore-user-config --skip-git-repo-check -c 'model_reasoning_effort="max"' -o /tmp/codex-effort-out --json` completed with `rc=0`; the invalid-value probe returned `Supported values are: 'none', 'minimal', 'low', 'medium', 'high', 'xhigh', and 'max'.` Firstmate's shared axis remains low..max. |

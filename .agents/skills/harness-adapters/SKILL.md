...
| codex | `--model <model>` | `-c 'model_reasoning_effort="<low\|medium\|high\|xhigh>"'`, plus `max` only on models that advertise it | Verified 2026-08-10 on codex-cli 0.147.0. Codex advertises reasoning levels **per model, not per harness**, so `max` is not a harness-level capability. `codex debug models` reports `low,medium,high,xhigh` for every catalog model and adds `max` only for `gpt-5.6-sol`, `gpt-5.6-sol-wm`, `gpt-5.6-terra`, `gpt-5.6-luna`, and `codex-auto-review`. Model-qualified probes with `printf 'Do not use tools. Reply OK.\\n' | codex exec --ephemeral --ignore-user-config --skip-git-repo-check --model <model> -c 'model_reasoning_effort="max"'`: `--model gpt-5.6-luna` ran to completion (`reasoning effort: max`, reply `OK`); `--model gpt-5.5` failed every turn with HTTP 400 `invalid_request_error/unsupported_value` - `'max' is not supported with the 'gpt-5.5-codex-1p-codexswic-ev3' model. Supported values are: 'none', 'low', 'medium', 'high', and 'xhigh'.` The earlier model-unqualified `rc=0` acceptance only exercised the configured default model (`gpt-5.6-sol`), and the client-side enum message listing `max` is a config parse check, not per-model support. `bin/fm-spawn.sh` therefore queries `codex debug models` for the selected model and omits `max` unless that model lists it. A spawn that pins no model resolves its identity through `codex doctor --json` (`.checks["config.load"].details.model`, codex's own resolver) and caps `max` at the standing reviewer level `medium` unless that resolved default is `gpt-5.6-luna`. Firstmate's shared axis remains low..max. |

...
A pull-based guard (`bin/fm-guard.sh`) warns through supervision tool output if the primary checkout is tangled, if work, process-event sources, or X-mode relay polling needs supervision without a watcher beacon fresh inside the guard grace window, or if queued wakes are waiting to be drained.
...
A ship or scout brief likewise pins its role charter by content hash, and the spawn refuses a hash that no longer matches the assembled `roles/dist/` charter, so the worker never runs charter text the task record does not pin; `bin/fm-brief.sh` and `bin/fm-spawn.sh` own the exact line format, defaults, and back-compat warning.

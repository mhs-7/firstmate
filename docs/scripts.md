...
| `fm-roles-lint.sh`       | Lint the role-charter tree; `--brief` is the reviewer-blinding check run before dispatching a second-vendor review |
...
| `fm-precheck.sh`         | Advisory local-model claims-vs-diff gap check before review seats dispatch (always exit 0) |

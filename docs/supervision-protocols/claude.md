...
8. The turn-end guard (`bin/fm-turnend-guard.sh --claude`) remains the final backstop.
   It requires a live identity-matched watcher lock plus a fresh beacon, a stricter predicate than the pull guard's beacon-freshness-only check, so a quiet handling-turn guard alone does not prove the stop will be allowed.
